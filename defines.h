...
#define LEN_WWW_USERNAME                                                       \
  65 // credentials for basic auth of server internal website

#define LEN_FS_SSID 33 // credentials for sensor access point mode

#define LEN_HEIGHT_ABOVE_SEALEVEL 8
#define LEN_TEMP_OFFSET 5
 ...
 * Constants                                                      *
 ******************************************************************/
constexpr const unsigned long SAMPLETIME_MS =
    30000; // time between two measurements of the PPD42NS
constexpr const unsigned long SAMPLETIME_SDS_MS =
    1000; // time between two measurements of the SDS011, PMSx003, Honeywell PM
          // sensor
constexpr const unsigned long WARMUPTIME_SDS_MS =
    15000; // time needed to "warm up" the sensor before we can take the first
           // measurement
constexpr const unsigned long READINGTIME_SDS_MS =
    5000; // how long we read data from the PM sensors

constexpr const unsigned long SAMPLETIME_NPM_MS = 60000;

constexpr const unsigned long SAMPLETIME_MHZ16_MS = 10000;
constexpr const unsigned long SAMPLETIME_S88_MS = 10000;   // SensAir S88
constexpr const unsigned long SAMPLETIME_MHZ19_MS = 10000; // MH-Z19C
constexpr const unsigned long SAMPLETIME_CCS811_MS = 10000;
constexpr const unsigned long SAMPLETIME_NEBULEAIR_MS = 120000; // 2 minutes

constexpr const unsigned long SAMPLETIME_GPS_MS = 50;
constexpr const unsigned long DISPLAY_UPDATE_INTERVAL_MS =
    5000; // time between switching display to next "screen"
constexpr const unsigned long ONE_DAY_IN_MS = 24 * 60 * 60 * 1000;
constexpr const unsigned long PAUSE_BETWEEN_UPDATE_ATTEMPTS_MS =
    ONE_DAY_IN_MS; // check for firmware updates once a day
constexpr const unsigned long DURATION_BEFORE_FORCED_RESTART_MS =
    ONE_DAY_IN_MS * 28; // force a reboot every ~4 weeks

// GPIO Pins
//  the IO pins which can be used for what depends on the following:
//    - The board which is used
//      - onboard peripherials like LCD or LoRa chips which already occupy an IO
//      pin
...
//      - configuration of ESP32 module configuration options ("strapping") like
//      operating voltage and boot medium
